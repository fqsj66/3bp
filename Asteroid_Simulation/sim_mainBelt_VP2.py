...
while True:
    ...
    #    q1 = mp.Queue()
    #    p1 = mp.Process(target=evolve_V_P, args=(xs, ys, zs, vxs, vys, vzs, N_start, timestep, T, Sol_M_J_a_Gravity_V,q1,))
    #    p1.start()
    #    results = q1.get() #Unload queue before joining process
    #    print("""
#RESULTS START
#""")
        #print(results)
...
#""")
    ...
    for i in range(0, len(xs)): #Not efficient but only happens once at the end of each run
        if xs[i] < 1E13 and ys[i] < 1E13: #Doesn't save particles that are outside solar system (> 100 AU)
